...
```{r}
# Set rootpaths
rp <- '/Users/Franciscorowe 1/Dropbox/Francisco/Research/grants/2020/iom/github/iom_project/'
dp <- 'data/tweet_data/'
mp <- 'methods/'
...
summarise_data <- function(x,y) {
  ...
  # Join data
  df <- cbind(x$date, y, x$created_at, x$status_id, x$weightings, x$cntry_id, x$lexicon_text, x$sentiment_text, x$retweeted_user)
...
rm_dupl_rt <- function(x) {
  ...
  rsid <- x$retweeted_user %>% 
    ...
    unname() %>% as.numeric()
  # Create vector of status id's of tweets in dataset
  sid <- x$status_id
  # if statement conditional on whether a retweet and it's original are present in the data
...
rm_unwanted_dupl <- function(x) {
  ...
  dpls <- x[duplicated(x$text) | duplicated(x$text, fromLast = TRUE),] 
  # Split into individual dataframes
  dpls_split <- dpls %>% split(.$text) 
  ...
  # Remove retweeted_user variable (no longer needed)
  x$retweeted_user <- NULL
  # Return output
  return(x)
}
...
ggsave(paste0(rp,"fig/topic_models/topic_models_fit.png"), dpi=300)
write.csv(result, paste0(rp,dp,"topic_models/topic_models_fit.csv")) # Save metrics
```
...
# Save outputs
#write.csv(lda_topics, file = paste0(rp,dp,'topic_models\\tweet_topic_betas_7.csv'))
#write.csv(lda_docs, file = paste0(rp,dp,'topic_models\\tweet_topic_gammas_7.csv'))
#write.csv(lda_topics, file = paste0(rp,dp,'topic_models\\tweet_topic_betas_9.csv'))
#write.csv(lda_docs, file = paste0(rp,dp,'topic_models\\tweet_topic_gammas_9.csv'))
#write.csv(lda_topics, file = paste0(rp,dp,'topic_models\\tweet_topic_betas_11.csv'))
#write.csv(lda_docs, file = paste0(rp,dp,'topic_models\\tweet_topic_gammas_11.csv'))
#write.csv(lda_topics, file = paste0(rp,dp,'topic_models\\tweet_topic_betas_12.csv'))
#write.csv(lda_docs, file = paste0(rp,dp,'topic_models\\tweet_topic_gammas_12.csv'))
#write.csv(lda_topics, file = paste0(rp,dp,'topic_models\\tweet_topic_betas_13.csv'))
#write.csv(lda_docs, file = paste0(rp,dp,'topic_models\\tweet_topic_gammas_13.csv'))
#write.csv(lda_topics, file = paste0(rp,dp,'topic_models\\tweet_topic_betas_14.csv'))
#write.csv(lda_docs, file = paste0(rp,dp,'topic_models\\tweet_topic_gammas_14.csv'))
write.csv(lda_topics, file = paste0(rp,dp,'topic_models\\tweet_topic_betas_15.csv'))
write.csv(lda_docs  , file = paste0(rp,dp,'topic_models\\tweet_topic_gammas_15.csv'))
```

# Plot FindTopicsNumber Results
```{r, fig.width=14}
# Read in Results
result <- read_in_csv("topic_models/topic_models_fit.csv")
...
```{r}
# Save results
ggsave(paste0(rp,'fig\\topic_models\\topic_models_fit.png'), width = 11, height = 8, dpi=300)
svg(paste0(rp,'fig\\topic_models\\topic_models_fit.svg'), width = 33, height = 24)
ggplot(result, aes(topics, score, color = method)) +
  geom_line(size = 1) +
  geom_point(size = 1.2) +
  facet_wrap(~ min, nrow = 2, scales = "free") +
...
lda_topics <- read.csv(file = paste0(rp,dp,'topic_models/tweet_topic_betas_',topics,'.csv'))
lda_topics$X <- NULL
...
lda_top_terms <- lda_topics %>% 
  ...
  top_n(20, beta) %>% # Select ten most important terms
  ungroup() %>%
  arrange(topic, -beta)
...
```{r}
# Save results
ggsave(paste0(rp,'fig/topic_models/topic_term_prob_',topics,'.png'), width = 8, height = 8, dpi=300)
svg(paste0(rp,'fig/topic_models/topic_term_prob_',topics,'.svg'), width = 24, height = 24)
lda_top_terms %>% 
  mutate(term = reorder_within(term, beta, topic)) %>% 
  ggplot(aes(term, beta, fill = factor(topic))) +
  geom_col(show.legend = FALSE) +
...
lda_docs <- read.csv(file = paste0(rp,dp,'topic_models\\tweet_topic_gammas_',topics,'.csv'))
lda_docs$X <- NULL
...
lda_agg2 %>% 
  filter(cntry_id == 4) %>% 
  filter(between(created_at, as.POSIXct("2020-02-11 00:00:00"), as.POSIXct("2020-02-13 00:00:00") )) %>% 
  arrange(-weightings)

lda_agg2 %>% 
  filter(cntry_id == 4) %>% 
  filter(between(created_at, as.POSIXct("2020-04-09 00:00:00"), as.POSIXct("2020-04-12 00:00:00") )) %>% 
  arrange(-weightings)

lda_agg2 %>% 
  filter(cntry_id == 4) %>% 
  filter(between(created_at, as.POSIXct("2019-12-18 00:00:00"), as.POSIXct("2019-12-20 00:00:00") )) %>% 
  arrange(-weightings)

lda_agg2 %>% 
  filter(cntry_id == 3) %>% 
  filter(between(created_at, as.POSIXct("2020-02-08 00:00:00"), as.POSIXct("2020-02-10 00:00:00") )) %>% 
  arrange(-weightings)

lda_agg2 %>% 
  filter(cntry_id == 3) %>% 
  filter(between(created_at, as.POSIXct("2020-04-23 00:00:00"), as.POSIXct("2020-04-25 00:00:00") )) %>% 
  arrange(-weightings)
...
lda_agg2 %>% 
  filter(cntry_id == 1) %>% 
  filter(between(created_at, as.POSIXct("2020-01-30 00:00:00"), as.POSIXct("2020-02-01 00:00:00") )) %>% 
  arrange(-weightings)

lda_agg2 %>% 
  filter(cntry_id == 1) %>% 
  filter(between(created_at, as.POSIXct("2020-02-19 00:00:00"), as.POSIXct("2020-02-20 00:00:00") )) %>% 
  arrange(-weightings)

lda_agg2 %>% 
  filter(cntry_id == 2) %>% 
  filter(between(created_at, as.POSIXct("2020-03-17 00:00:00"), as.POSIXct("2020-03-19 00:00:00") )) %>% 
  arrange(-weightings)

lda_agg2 %>% 
  filter(cntry_id == 2) %>% 
  filter(between(created_at, as.POSIXct("2020-04-20 00:00:00"), as.POSIXct("2020-04-22 00:00:00") )) %>% 
  arrange(-weightings)
...
```{r}
# Save results
#ggsave(paste0(rp,'fig\\topic_models\\daily_topics_perc_',topics,'.png'), width = 5, height = 6.5, dpi=300)
ggsave(paste0(rp,'fig\\topic_models\\daily_topics_perc_',topics,'_from_15_exp.png'), width = 5, height = 6.5, dpi=300)
svg(paste0(rp,'fig\\topic_models\\daily_topics_perc_',topics,'_from_15_exp.svg'), width = 15, height = 19.5)
grman_dp + italy_dp + spain_dp + uk_dp + usa_dp + plot_layout(nrow = 5) & theme(legend.key.size = unit(0.25, 'cm'), legend.box.spacing = unit(0.1, 'cm'))
dev.off()
```
...
```{r}
# Save results
ggsave(paste0(rp,'fig\\topic_models\\daily_topics_count_',topics,'_from_15_exp.png'), width = 7, height = 8, dpi=300)
```
...
# Plot
plot_topic_daily_perc <- function(df,title,topics,lab="-",svg=T,leg=F) {
  # Main plot
  p <- ggplot(df, aes(fill = topic, y=daily_perc, x=day)) + 
    geom_bar(position="stack", stat="identity") + 
    theme_tufte() + 
  ...
  # If png, reduce lineheight
  if (svg == F) {
    p <- p + theme(text = element_text(lineheight = 1))
  }
...
# Define positive png plots
grman_dp_pos_png <- plot_topic_daily_perc(grman_daily_topics_pos,'Positive\nGermany',topics,svg=F)
italy_dp_pos_png <- plot_topic_daily_perc(italy_daily_topics_pos,'Italy'            ,topics,svg=F)
spain_dp_pos_png <- plot_topic_daily_perc(spain_daily_topics_pos,'Spain'            ,topics,svg=F,lab='y')
uk_dp_pos_png    <- plot_topic_daily_perc(uk_daily_topics_pos,   'United Kingdom'   ,topics,svg=F)
usa_dp_pos_png   <- plot_topic_daily_perc(usa_daily_topics_pos,  'United States'    ,topics,svg=F,lab='x')
# Define negative png plots
grman_dp_neg_png <- plot_topic_daily_perc(grman_daily_topics_neg,'Negative\n'       ,topics,svg=F)
italy_dp_neg_png <- plot_topic_daily_perc(italy_daily_topics_neg,' '                ,topics,svg=F)
spain_dp_neg_png <- plot_topic_daily_perc(spain_daily_topics_neg,' '                ,topics,svg=F,leg = TRUE)
uk_dp_neg_png    <- plot_topic_daily_perc(uk_daily_topics_neg,   ' '                ,topics,svg=F)
usa_dp_neg_png   <- plot_topic_daily_perc(usa_daily_topics_neg,  ' '                ,topics,svg=F,lab='x')

# Define positive svg plots
grman_dp_pos_svg <- plot_topic_daily_perc(grman_daily_topics_pos,'Positive\nGermany',topics)
italy_dp_pos_svg <- plot_topic_daily_perc(italy_daily_topics_pos,'Italy'            ,topics)
spain_dp_pos_svg <- plot_topic_daily_perc(spain_daily_topics_pos,'Spain'            ,topics,lab='y')
uk_dp_pos_svg    <- plot_topic_daily_perc(uk_daily_topics_pos,   'United Kingdom'   ,topics)
usa_dp_pos_svg   <- plot_topic_daily_perc(usa_daily_topics_pos,  'United States'    ,topics,lab='x')
# Define negative svg plots
grman_dp_neg_svg <- plot_topic_daily_perc(grman_daily_topics_neg,'Negative\n'       ,topics)
italy_dp_neg_svg <- plot_topic_daily_perc(italy_daily_topics_neg,' '                ,topics)
spain_dp_neg_svg <- plot_topic_daily_perc(spain_daily_topics_neg,' '                ,topics,leg = TRUE)
uk_dp_neg_svg    <- plot_topic_daily_perc(uk_daily_topics_neg,   ' '                ,topics)
usa_dp_neg_svg   <- plot_topic_daily_perc(usa_daily_topics_neg,  ' '                ,topics,lab='x')
```


```{r, fig.width=14,fig.height=14}
((grman_dp_pos_png|grman_dp_neg_png) / (italy_dp_pos_png|italy_dp_neg_png) / (spain_dp_pos_png|spain_dp_neg_png) / (uk_dp_pos_png|uk_dp_neg_png) / (usa_dp_pos_png|usa_dp_neg_png)) + plot_layout(nrow = 5) & theme(legend.key.size = unit(0.25, 'cm'), legend.box.spacing = unit(0.1, 'cm'))
```
...
# Save svg file
svg(paste0(rp,'fig\\topic_models\\daily_topics_perc_',topics,'_pn_exp.svg'), width = 19.5, height = 19.5)
((grman_dp_pos_svg|grman_dp_neg_svg) / (italy_dp_pos_svg|italy_dp_neg_svg) / (spain_dp_pos_svg|spain_dp_neg_svg) / (uk_dp_pos_svg|uk_dp_neg_svg) / (usa_dp_pos_svg|usa_dp_neg_svg)) + plot_layout(nrow = 5) & theme(legend.key.size = unit(0.25, 'cm'), legend.box.spacing = unit(0.1, 'cm'))
dev.off()
```
