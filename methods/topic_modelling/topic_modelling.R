...
#all_tweets <- read.csv("./Data/Twitter/lockdown_misinformation_merge.csv", header = TRUE)
...
misinformation <- read_in_csv('tweets\\uk_tweets_01122019_01052020_VADER_removed_dpl_RT_and_status_id_only.csv')

#misinformation <- all_tweets[all_tweets$misinformation == 1,] # Subset only tweets identified as misinformation
#rm(all_tweets)

# Hold date for later
timestamp <- misinformation[,c("status_id", "created_at")]
...
hold <- misinformation[,c("status_id", "VADER_text")] # Subset required information
# Corpus requires following column names
names(hold)[names(hold) == "status_id"] <- "doc_id"
names(hold)[names(hold) == "VADER_text"] <- "text"
...
ggsave(paste0(rp,"fig\\topic_models_fit.png"), dpi=300)
write.csv(result, paste0(rp,dp,"topic_models\\topic_models_fit.csv")) # Save metrics

# Classify tweets
lda_results <- LDA(dtm_new, k = 7, control = list(seed = 281190), method = "Gibbs")
...
lda_top_terms %>% 
  ...
  coord_flip() +
  scale_x_reordered()
ggsave(paste0(rp,"fig\\topic_term_prob_7.png"), dpi=300)
